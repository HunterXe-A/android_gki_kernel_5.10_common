...
static ssize_t hwid_sku_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{ return snprintf(buf, PAGE_SIZE, "%s\n", sku); }
static ssize_t hwid_country_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{ return snprintf(buf, PAGE_SIZE, "%s\n", country); }
static ssize_t hwid_level_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{ return snprintf(buf, PAGE_SIZE, "%s\n", level); }
static ssize_t hwid_version_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{ return snprintf(buf, PAGE_SIZE, "%s\n", version); }

const char *get_hw_sku(void) { return sku; }
EXPORT_SYMBOL(get_hw_sku);
const char *get_hw_country(void) { return country; }
EXPORT_SYMBOL(get_hw_country);
const char *get_hw_level(void) { return level; }
EXPORT_SYMBOL(get_hw_level);
const char *get_hw_version(void) { return version; }
...
static struct attribute *hwid_attrs[] = {
	&hwid_sku_attr.attr, &hwid_country_attr.attr,
	&hwid_level_attr.attr, &hwid_version_attr.attr, NULL,
};
static struct attribute_group attr_group = { .attrs = hwid_attrs };

static int __init hwid_module_init(void)
{
	int ret = -ENOMEM;
	hwid_kobj = kobject_create_and_add("hwid", NULL);
	if (!hwid_kobj) { pr_err("hwid: init failed\n"); goto fail; }
	ret = sysfs_create_group(hwid_kobj, &attr_group);
	if (ret) { pr_err("hwid: sysfs register failed\n"); goto sys_fail; }
sys_fail: kobject_del(hwid_kobj);
fail: return ret;
}

static void __exit hwid_module_exit(void)
{
	if (hwid_kobj) { sysfs_remove_group(hwid_kobj, &attr_group); kobject_del(hwid_kobj); }
}
